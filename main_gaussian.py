...
class Model:
    ...
    def sample_and_evaluate(self, num_samples, epoch=args.epochs - 1, result_path=None):
        ...
        # print(time.time() - t1)
        samples_np = binary_list[-1].astype(np.int32)  # 转换为 NumPy 数组
        # 将样本展开为二维数组，形状为 [num_samples, N]
        ...
        print(f"generate {num_samples} samples, KL divergence: {kl_0:.5f}")
        ...
        if not args.fast_flat:
            with open(output_file_path, 'w') as f:
                for sample in samples_flat:
                    # 将每个样本转换为字符串，并用空格分隔，然后写入文件
                    sample_str = ''.join(map(str, sample))  # 例如：将[0, 1, 0, 1]转换为"0101"
                    f.write(sample_str + '\n')  # 每个样本作为一行写入文件

        # 画最原始的均匀分布，这些样本是从均匀分布中采出来的
    ...
    def draw_picture(self, frequencies, result_path, epoch, kl, t, inf_flat=False):
        ...
        plt.figure(figsize=(10, 6))  # 增加图表大小
